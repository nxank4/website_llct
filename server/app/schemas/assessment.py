...
class AssessmentBase(BaseModel):
    ...
    max_attempts: Optional[int] = (
        None  # None hoặc 0 = không giới hạn, > 0 = giới hạn số lần
    )
    is_published: bool = False
    is_randomized: bool = False
    show_results: bool = True  # Allow students to see correct answers after completion
    show_explanations: bool = (
        True  # Allow students to see explanations after completion
    )
...
class AssessmentUpdate(BaseModel):
    ...
    assessment_type: Optional[
        Literal["pre_test", "post_test", "quiz", "exam", "assignment"]
    ] = None
...
class QuestionBase(BaseModel):
    ...
    allow_multiple_selection: Optional[bool] = (
        False  # For multiple_choice: allow selecting multiple options
    )
    word_limit: Optional[int] = None  # For essay: maximum word count (None = unlimited)
    input_type: Optional[Literal["text", "number"]] = (
        None  # For essay/fill_in_blank: expected input type
    )
