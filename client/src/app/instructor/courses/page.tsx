...
export default function InstructorCoursesPage() {
  ...
  const handleDeleteCourse = (courseId: number) => {
    setDeleteConfirmDialog({ isOpen: true, courseId });
  };

  const confirmDeleteCourse = () => {
    if (!deleteConfirmDialog.courseId) return;

    try {
      setCourses((prev) =>
        prev.filter((course) => course.id !== deleteConfirmDialog.courseId)
      );
      setDeleteConfirmDialog({ isOpen: false, courseId: null });
    } catch (error) {
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  ...
                  <Select value={filterStatus} onValueChange={setFilterStatus}>
                    <SelectTrigger className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white w-[150px]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() =>
                          handlePublishToggle(course.id, course.is_published)
                        }
