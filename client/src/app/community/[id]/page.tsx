...
export default function CommunityPostDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const p = await fetch(
          `http://127.0.0.1:8000/api/v1/community/posts/${postId}`
        ).then((r) => r.json());
        const c = await fetch(
          `http://127.0.0.1:8000/api/v1/community/posts/${postId}/comments`
        ).then((r) => r.json());
        if (!cancelled) {
          setPost(p);
          setComments(c);
  ...
  const onLike = async () => {
    await fetch(`http://127.0.0.1:8000/api/v1/community/posts/${postId}/like`, {
      method: "POST",
    });
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <button
            onClick={() => router.back()}
            className="text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 flex items-center mb-4"
          >
            <ArrowLeft className="h-5 w-5 mr-1" />
            Quay lại
          </button>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8 text-center text-gray-900 dark:text-white">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center text-gray-600 dark:text-gray-400">
              <MessageCircle className="h-4 w-4 mr-1" /> {post.comments_count}{" "}
              bình luận
            </div>
          </div>
        </div>
